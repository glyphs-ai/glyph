...
const files = new Map([["SKILL.md", Buffer.from("# body")]]);

describe("SkillManifest.create", () => {
  it("accepts compliant frontmatter and defaults scope to public", () => {
    const r = SkillManifest.create({ name: "tool-use", description: "d", version: "1.0.0" }, files);
  ...
  it("keeps an explicit scope", () => {
    const r = SkillManifest.create(
      { name: "tool-use", scope: "com.acme", description: "d", version: "1.0.0" },
      files,
    );
  ...
  it("rejects a missing version", () => {
    const r = SkillManifest.create({ name: "tool-use", description: "d" }, files);
    expect(r.isErr()).toBe(true);
    if (r.isErr()) expect(r.error.type).toBe("SkillManifestInvalid");
  });

  it("rejects an uppercase name", () => {
    const r = SkillManifest.create({ name: "ToolUse", description: "d", version: "1.0.0" }, files);
    expect(r.isErr()).toBe(true);
  });
});
