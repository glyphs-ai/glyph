...
import { AgentFrontmatterError, AgentNameInvalidError } from "../src/domain/agent.errors.js";
...
describe("AgentEntity.create", () => {
  it("returns an entity with derived FQN and exposed metadata", () => {
    const a = AgentEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:/abs/agents/researcher")),
      "test",
    );
  ...
  it("normalizes the origin to canonical form", () => {
    const a = AgentEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:F:\\path\\researcher")),
      "test",
    );
    expect(a.origin).toBe("file:///F:/path/researcher");
    const b = AgentEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:F:/path/researcher")),
      "test",
    );
  ...
  it("propagates frontmatter errors", () => {
    expect(() =>
      AgentEntity.create("# no frontmatter\n", Origin.parse(safeNormalize("file:/abs/x")), "test"),
  ...
  it("propagates name validation errors", () => {
    expect(() =>
      AgentEntity.create(
        MIN_VALID.replace("researcher", "BadName"),
...
describe("AgentEntity.withAnchor", () => {
  it("returns a new entity with updated metadata, preserved identity", () => {
    const a1 = AgentEntity.create(MIN_VALID, Origin.parse(safeNormalize("file:/abs/x")), "test");
