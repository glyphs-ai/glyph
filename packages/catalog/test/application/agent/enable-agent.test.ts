...
import { EnableAgentUseCase } from "../../../src/application/agent/enable-agent.js";
...
let useCase: EnableAgentUseCase;

beforeEach(() => {
  agentRepo = mock<AgentRepository>();
  agentRepo.save.mockReturnValue(okAsync(undefined));
  useCase = new EnableAgentUseCase({ agentRepo });
});

describe("EnableAgentUseCase — happy path", () => {
  it("enables a disabled agent, saves it, and returns the projected Agent DTO", async () => {
    ...
    expect(entity.disabledByUser).toBe(false);
    expect(agentRepo.save).toHaveBeenCalledWith(entity);
    expect(dto).toEqual(agentDto(entity));
  });

  it("is idempotent when the agent is already enabled", async () => {
    const entity = agentEntity();
    agentRepo.get.mockReturnValue(okAsync(entity));

    const dto = (await useCase.execute({ id: AGENT_ID }))._unsafeUnwrap();

    expect(dto.disabledByUser).toBe(false);
    expect(agentRepo.save).toHaveBeenCalledWith(entity);
  });
});

describe("EnableAgentUseCase — error channel", () => {
  ...
  it("propagates DatabaseUnavailable from repo.save", async () => {
    agentRepo.get.mockReturnValue(okAsync(agentEntity({ disabledByUser: true })));
