...
import { SkillNameInvalidError } from "../src/domain/skill.errors.js";
...
describe("SkillEntity.create", () => {
  it("returns an entity with derived FQN and exposed metadata", () => {
    const s = SkillEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:/abs/skills/tool-use")),
      "test",
    );
  ...
  it("normalizes the origin to canonical form", () => {
    const s = SkillEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:F:\\path\\skills\\tool-use")),
      "test",
    );
    expect(s.origin).toBe("file:///F:/path/skills/tool-use");
    const t = SkillEntity.create(
      MIN_VALID,
      Origin.parse(safeNormalize("file:F:/path/skills/tool-use")),
      "test",
    );
...
---
`;
    const s = SkillEntity.create(src, Origin.parse(safeNormalize("file:/abs/parent")), "test");
...
describe("SkillEntity.withAnchor", () => {
  it("returns a new entity with updated metadata, preserved identity", () => {
    const s1 = SkillEntity.create(MIN_VALID, Origin.parse(safeNormalize("file:/abs/x")), "test");
...
describe("SkillEntity.toJSON", () => {
  it("emits the wire shape (no scope/shortName/anchorContent, with timestamps)", () => {
    const s = SkillEntity.create(MIN_VALID, Origin.parse(safeNormalize("file:/abs/x")), "test");
    const json = s.toJSON();
