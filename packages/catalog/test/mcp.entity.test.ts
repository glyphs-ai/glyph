import { describe, expect, it } from "vitest";
import { McpEntity } from "../src/domain/mcp.entity.js";
import { McpInvalidJsonError, McpNameInvalidError } from "../src/domain/mcp.errors.js";
import * as McpFormat from "../src/domain/mcp.format.js";
import { InvalidOriginError, Origin } from "../src/domain/origin.js";
import { safeNormalize } from "../src/fetcher/origin.js";

describe("McpEntity.create", () => {
  it("validates name and origin, returns an entity with parseable spec", () => {
    const m = McpEntity.create(
      "azure/mcp",
      Origin.parse(safeNormalize("file:/abs/azure")),
      '{"command":"node"}',
    );
    expect(m.fqn).toBe("azure/mcp");
    expect(m.origin).toBe("file:///abs/azure");
    expect(m.installedAt).toBeTypeOf("string");
    const { meta, body } = McpFormat.parse(m.spec, "test");
    expect(meta).toEqual({ name: "azure/mcp" });
    expect(body.command).toBe("node");
  });

  it("normalizes the origin to canonical form", () => {
    const m = McpEntity.create(
      "azure/mcp",
      Origin.parse(safeNormalize("file:F:\\path\\mcps\\azure.json")),
      "{}",
    );
    expect(m.origin).toBe("file:///F:/path/mcps/azure.json");
    const n = McpEntity.create(
      "azure/mcp",
      Origin.parse(safeNormalize("file:F:/path/mcps/azure.json")),
      "{}",
    );
    expect(n.origin).toBe(m.origin);
  });

  it("injects _meta.name when input lacks one", () => {
    const m = McpEntity.create(
      "x/y",
      Origin.parse(safeNormalize("file:/abs/x")),
      '{"command":"node"}',
    );
    const parsed = JSON.parse(m.spec);
    expect(parsed._meta).toEqual({ name: "x/y" });
    expect(parsed.command).toBe("node");
  });

  it("rejects invalid name", () => {
    expect(() =>
      McpEntity.create("no-slash", Origin.parse(safeNormalize("file:/abs/x")), "{}"),
    ).toThrow(McpNameInvalidError);
    expect(() =>
      McpEntity.create("two/slashes/here", Origin.parse(safeNormalize("file:/abs/x")), "{}"),
    ).toThrow(McpNameInvalidError);
  });

  it("rejects empty origin", () => {
    expect(() => Origin.parse("")).toThrow(InvalidOriginError);
  });

  it("rejects unparseable content", () => {
    expect(() =>
      McpEntity.create("x/y", Origin.parse(safeNormalize("file:/abs/x")), "{not json"),
    ).toThrow(McpInvalidJsonError);
  });

  it("accepts empty content (creates fresh _meta-only object)", () => {
    const m = McpEntity.create("x/y", Origin.parse(safeNormalize("file:/abs/x")), "");
    const parsed = JSON.parse(m.spec);
    expect(parsed._meta).toEqual({ name: "x/y" });
  });
});

describe("McpEntity.fromStored", () => {
  it("trusts persisted spec (no parse, no inject)", () => {
    const stored = '{"raw":"stored","_meta":{"name":"x/y"}}\n';
    const now = "2026-05-19T00:00:00.000Z";
    const m = McpEntity.fromStored({
      fqn: "x/y",
      origin: "file:/abs/x",
      spec: stored,
      installedAt: now,
      updatedAt: now,
    });
    expect(m.spec).toBe(stored);
    expect(m.installedAt).toBe(now);
  });

  it("still validates the name", () => {
    const now = "2026-05-19T00:00:00.000Z";
    expect(() =>
      McpEntity.fromStored({
        fqn: "no-slash",
        origin: "file:/abs/x",
        spec: "{}",
        installedAt: now,
        updatedAt: now,
      }),
    ).toThrow(McpNameInvalidError);
  });
});

describe("McpEntity.withContent", () => {
  it("returns a new entity with replaced spec, identity preserved", () => {
    const m1 = McpEntity.create("x/y", Origin.parse(safeNormalize("file:/abs/x")), '{"v":1}');
    const m2 = m1.withContent('{"v":2}');
    expect(m2.fqn).toBe(m1.fqn);
    expect(m2.origin).toBe(m1.origin);
    expect(JSON.parse(m2.spec).v).toBe(2);
  });
});

describe("McpEntity.toJSON", () => {
  it("emits the wire shape: fqn (not name), no spec, with timestamps", () => {
    const m = McpEntity.create("x/y", Origin.parse(safeNormalize("file:/abs/x")), "{}");
    const json = m.toJSON();
    expect(json).toHaveProperty("fqn", "x/y");
    expect(json).toHaveProperty("installedAt");
    expect(json).toHaveProperty("updatedAt");
    expect(json).not.toHaveProperty("name");
    expect(json).not.toHaveProperty("spec");
  });
});
