...
export function workspacesRoutes(application: Application): Hono {
  ...
  app.get(
    "/",
    defineHandler("workspaces.list", async (c) => {
      try {
  ...
  app.get(
    "/current",
    defineHandler("workspaces.current.get", async (c) => {
      try {
        return { id: await service.getLastOpenedId() };
      } catch (err) {
        return respondError(c, err, {
  ...
  app.put(
    "/current",
    defineHandler("workspaces.current.set", async (c) => {
  ...
  app.get(
    "/:id",
    defineHandler("workspaces.get", async (c) => {
  ...
  app.patch(
    "/:id",
    defineHandler("workspaces.update", async (c) => {
      const id = c.req.param("id");
  ...
  app.delete(
    "/:id",
    defineHandler("workspaces.delete", async (c) => {
      const id = c.req.param("id");
  ...
  app.post(
    "/:id/reload",
    defineHandler("workspaces.reload", async (c) => {
      const id = c.req.param("id");
      try {
