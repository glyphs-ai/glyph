...
async function makeHarness(): Promise<Harness> {
  ...
  return { cap, application: sys.application, workspace: sys.workspace };
}
...
async function registerWs(
  ...
  return { id: result.value.id, workspaceDir: path.resolve(args.workspaceDir) };
}
...
describe("WorkspaceContext observability", () => {
  it("emits an info line on first container build, with workspaceId + workspaceDir", async () => {
    const { cap, application, workspace } = await makeHarness();
    const ws = await registerWs(workspace, {
      name: "alpha",
  ...
  it("emits an info line on invalidate of a loaded entry (via rename)", async () => {
    ...
    await application.renameWorkspace(ws.id, { name: "alpha-renamed" as WorkspaceName });
