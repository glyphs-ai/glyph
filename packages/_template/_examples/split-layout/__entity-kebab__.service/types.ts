...
// either placement is fine — pick one and use it consistently inside the split.
...
export interface __Entity__Repository {
  findAll(opts: List__Entity__Opts): Promise<__Entity__[]>;
  findById(id: string): Promise<__Entity__ | undefined>;
  insert(entity: __Entity__): Promise<void>;
 ...
 * `mutations.ts` writes and `queries.ts` reads).
 */
