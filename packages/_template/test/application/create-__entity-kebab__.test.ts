...
describe("Create__Entity__UseCase — error channel", () => {
  it("propagates __Entity__IdConflict from repo.insert", async () => {
    repo.insert.mockReturnValue(
      errAsync({ type: "__Entity__IdConflict", id: "X" as __Entity__Id }),
    );
    const res = await useCase.execute({ name: "Demo" });
    expect(res._unsafeUnwrapErr().type).toBe("__Entity__IdConflict");
  });
